...
export const useWebSocket = () => {
  ...
  const sendGetCampaignAddressRequest = (tokenContractAddress) => {
    ...
    try {
      console.log(
        "sendGetCampaignAddressRequest called" + tokenContractAddress
      );
      console.log(
        "webSocketInstance",
      ...
      if (webSocketInstance?.readyState === WebSocket.OPEN) {
        ...
        console.log("sendGetCampaignAddressRequest made request");
        webSocketInstance.send(request);
        // Listen for response
        return new Promise((resolve) => {
          webSocketInstance.onmessage = (event) => {
            const data = event.data;
            if (data.startsWith("1;r!get_campaign_address;")) {
              const response = data.substring(
                "1;r!get_campaign_address;".length
              );
              console.log("get campaign address response:", response);
